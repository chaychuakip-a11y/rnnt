...
my $hdir         = "/workdir/asrdictt/tasrdictt/zhyou2/mlg/202311_car/french/dnnfa_fixmlf_addfangzhen_all_fbnocmn40";
my $nSplit       = 10;
my $nPart        = 100;
my $states_list  = "/raw15/asrdictt/permanent/zhyou2/202311_mlg_car/french/0.data_prepare/mle_yingchen15/cdphone.map.nosp.list";
my $scp_filter   = "";
...
##out
my $dir_pwd      = '/yrfs4/asrdictt/zhyou2/traindata/mlg20231115/french';
#`pwd`; chomp($dir_pwd); $dir_pwd =~ s#/work1/#/yrfs4/#;
...
# my $bin_qnfiletransfer = "/work1/asrdictt/taoyu/tools/bin/qnfiletransfer_1";	##triphone fa
...
foreach my $split_id(@split_id)
{
	...
	# if(!-e "$pfile_fea.done.finish")
	{
		...
		$cmd = "/home3/hadoop/hadoop2/bin/hdfs dfs $dfs_param -cat $hdir_src |$bin_qnfiletransfer - $descript_fea - $descript_lab $pfile_lab $states_list $scp_lab $IsCTClab $scp_filter >$trans_log 2>&1";
	...
	if(!-e "$file_norm.done")
	{
		!system("source /home3/asrdictt/taoyu/cuda6.5.bashrc; $bin_qnnorm norm_ftrfile=$pfile_fea output_normfile=$file_norm") || die "qnnorm failed: $file_norm.\n";
