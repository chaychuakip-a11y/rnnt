...
# 匈牙利语原始数据 HDFS 路径，运行时可通过命令行参数覆盖：
#   perl 1_dnnfa.pl /your/hdfs/path/hungarian_data
my @hdir_src; if (@ARGV > 0) {
    @hdir_src = ($ARGV[0]);
} else {
    @hdir_src = (
                 $config_data->{hdfs_src_root},  # /workdir/asrdictt/dasrdictt/tyliu23/hu/src_wav/tongyong
                );
}
my $hdir_out = "$config_data->{hdfs_out_root}/hungarian_wav_dnnfa";
my $hdir_src = join(" -input ", @hdir_src);
my $dir_tmp  = "tmp"; mkdir $dir_tmp if !-e $dir_tmp;

## FA 资源路径（复用韩语 FA 模型，9004 states / 104 phones）
...
my (@cmd_map, @cmd_red, @files);
my ($cmd_map, $cmd_red, $files, $cmd);
...
@files = (
$config, $acmod_bin, $wts_file, $fea_norm, $state_count,
$bin_atom, $bin_libboost_so, $bin_libboost_thread_so, $bin_libboost_system_so,
$bin_libmkl_lp64_so, $bin_libmkl_thread_so, $bin_libmkl_core_so,
$bin_libiomp5_so, $bin_libimf_so, $bin_libsvml_so, $bin_libirng_so, $bin_libintlc_so,
$bin_lattice_so, $bin_decode_so, $bin_ulm_so, $bin_lm_so, $bin_res_so,
$fst_bin, $G_fst, $hmmlist,
$bin_cmvn, $bin_raw_fea, $config1, $config2, $config3,
$bin_renametail, $bin_selecttail, $bin_randname, $bin_randnamered,
);
