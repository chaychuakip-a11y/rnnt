...
def run_multi_gpu(config, LOG):
    ...
    # torchintx.clamp_module(model.encoder.conv04, clamp_weight_value=1, clamp_bias_value=3, clamp_output_value=2, clamp_dynamic_percent=1.0)#100 50 30 8  ###压到30，可以放开前三行，fuse_bn=True
    # ####step3：逐层调，需要先关闭output，单独调
    ####step1
