...
#spmodel="/train8/asrmlg/ddye2/RNNT/danish/am/5_rnnt/1_spm/model_500/spm_da_bpe_500.model"
...
#mlp_dir = "/train8/asrmlg/ddye2/RNNT/russian/russian_gongban_20241101/4_train_rnnt/train_alldata_init-en-lr0.08_phone_rand/best_model"
#test_model_name = mlp_dir + "/model.iter{0}.part{1}".format(iter_idx, part_idx)
#test_model_name = mlp_dir + "/model.iter13.part3"
test_model_name =sys.argv[1]
print(test_model_name)
#test_model_name = "/train8/asrmlg/ddye2/RNNT/russian/russian_gongban_finetune_gbcz_20250219_decoderand_tts2x_mulpfile_new_sjzc_czclamp/4_train_notts/out_train_001_ctcclamp/model.iter7.part1"
...
if __name__ == "__main__":
    with torch.cuda.device(device):
        ...
        model.eval()
        #print(model)
        #conv与bn合为一体，两组参数变为一组参数，减少计算量
        torchintx.trace_layers(model, model.encoder, aa, fuse_bn=True )

        #一些模块由官方的变成自己封装的模块，便于量化，此处conv与bn为替换，可能原因是上面trace_layers让两者融合在一起，未匹配。
        # 自此输出只有模型结构改变，基本上参数不变。
        ###############========================================================############
        ##torchintx.clamp_module(model.encoder.lstm, clamp_weight_value=8, clamp_bias_value=8, clamp_output_value=0.2, clamp_dynamic_percent=1.0)
        ##-----------l1
        #torchintx.clamp_module(model.encoder.lstm, clamp_weight_value=(5.8437,8), clamp_bias_value=(6.8442,6.8345), clamp_output_value=0.3, clamp_dynamic_percent=1.0)
        ##torchintx.clamp_module(model.encoder.lstm2, clamp_weight_value=8, clamp_bias_value=8, clamp_output_value=0.3, clamp_dynamic_percent=1.0)
        ##-----------l1
        #torchintx.clamp_module(model.encoder.lstm2, clamp_weight_value=(7.9997,7.9999), clamp_bias_value=(3.3030,3.3279), clamp_output_value=0.3, clamp_dynamic_percent=1.0)
        ##torchintx.clamp_module(model.encoder.lstm3, clamp_weight_value=8, clamp_bias_value=8, clamp_output_value=0.3, clamp_dynamic_percent=1.0)
        ##-----------l1
        #torchintx.clamp_module(model.encoder.lstm3, clamp_weight_value=(7.9999,7.9998), clamp_bias_value=(2.3620,2.3882), clamp_output_value=0.3, clamp_dynamic_percent=1.0)
        #-----------l2
        #torchintx.clamp_module(model.encoder.lstm, clamp_weight_value=(1.0910,7.9), clamp_bias_value=(6.8336,6.8267), clamp_output_value=0.0827, clamp_dynamic_percent=1.0)
        ###-----------l1
        ##torchintx.clamp_module(model.encoder.lstm2, clamp_weight_value=(7.9997,7.9999), clamp_bias_value=(3.3030,3.3279), clamp_output_value=0.3, clamp_dynamic_percent=1.0)
        ##-----------l2
        #torchintx.clamp_module(model.encoder.lstm2, clamp_weight_value=(7.9996,7.9998), clamp_bias_value=(2.8829,2.9045), clamp_output_value=0.0543, clamp_dynamic_percent=1.0)
        ###-----------l1
        ##torchintx.clamp_module(model.encoder.lstm3, clamp_weight_value=(7.9999,7.9998), clamp_bias_value=(2.3620,2.3882), clamp_output_value=0.3, clamp_dynamic_percent=1.0)
        ##-----------l2
        #torchintx.clamp_module(model.encoder.lstm3, clamp_weight_value=(7.9997,7.9994), clamp_bias_value=(2.2372,2.2610), clamp_output_value=0.0567, clamp_dynamic_percent=1.0)
        ##############========================================================############
