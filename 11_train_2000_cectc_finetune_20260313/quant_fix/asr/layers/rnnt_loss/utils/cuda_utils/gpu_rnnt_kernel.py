...
from asr.layers.rnnt_loss.utils import rnnt_helper
...
def logp(
    ...
    return denom[col] + acts[col * alphabet_size + v] #blank_ - 1 - i 
...
def compute_grad_kernel(
    ...
    if t < T and u < U:
        ...
        while idx < alphabet_size:
            ...
            grad = math.exp(alphas[col] + betas[col] + logpk - logll[mb]) # M
            ...
            if fastemit_lambda > 0.0 and u < U - 1:
                fastemit_grad = fastemit_lambda * math.exp( # M * fastemit_lambda*exp((denom[col] + acts[col * alphabet_size + labels[u]]))
            ...
            grad = grad + fastemit_grad # M + M * fastemit_lambda*exp((denom[col] + acts[col * alphabet_size + labels[u]]))
            ...
            if (idx == blank_) and (t < T - 1):
                ...
                # M(1+fastemit_lambda*N) - M = M(fastemit_lambda*N)
            ...
            if (u < U - 1) and (idx == labels[u]):
                ...
                # M(1+fastemit_lambda*N) - fastemit_lambda*M = M*(1+fastemit_lambda*(N-1))
    ...
    for n in range(1, T + U - 1):
        ...
        if u == 0:
            ...
            if t > 0 and t < T:
                ...
                # Now add the weights for big blanks. add 
                for i in range(num_big_blanks):
                    if t >= big_blank_duration[i]:
                        alphas[offset + t * maxU + u] = rnnt_helper.log_sum_exp(
                            ...
                            + logp(
                                denom, acts, maxT, maxU, alphabet_size, b, t - big_blank_duration[i], 0, alphabet_size - 1 - i
                            )
                            - sigma,
                        )
                ...
                for i in range(num_big_blanks):
                    if t >= big_blank_duration[i]:
                        ...
                        big_blank_no_emit = (
                            alphas[offset + (t - big_blank_duration[i]) * maxU + u]
                            + logp(
                                denom, acts, maxT, maxU, alphabet_size, b, t - big_blank_duration[i], u, alphabet_size - 1 - i
                            )
                            - sigma
                        )
                        alphas[offset + t * maxU + u] = rnnt_helper.log_sum_exp(
        ...
        for i in range(num_big_blanks):
            if T >= big_blank_duration[i]:
                big_blank_loglike = (
                    ...
                    + logp(denom, acts, maxT, maxU, alphabet_size, b, T - big_blank_duration[i], U - 1, alphabet_size - 1 - i)
                    - sigma
                )
                ...
                for i in range(num_big_blanks):
                    if t + big_blank_duration[i] < T:
                        ...
                        betas[offset + t * maxU + U - 1] = rnnt_helper.log_sum_exp(
                            ...
                            + logp(denom, acts, maxT, maxU, alphabet_size, b, t, U - 1, alphabet_size - 1 - i)
                            - sigma,
                        )
                        ...
                        betas[offset + t * maxU + U - 1] = rnnt_helper.log_sum_exp(
                            betas[offset + t * maxU + U - 1],
                            logp(denom, acts, maxT, maxU, alphabet_size, b, t, U - 1, alphabet_size - 1 - i) - sigma,
                        )
                ...
                for i in range(num_big_blanks):
                    if t < T - big_blank_duration[i]:
                        ...
                        big_blank_no_emit = (
                            ...
                            + logp(denom, acts, maxT, maxU, alphabet_size, b, t, u, alphabet_size - 1 - i)
                            - sigma
                        )
    ...
    if t < T and u < U:
        ...
        while idx < alphabet_size:
            ...
            if (idx == blank_) and (t == T - 1) and (u == U - 1):
                ...
                for i in range(num_big_blanks):
                    if (idx == alphabet_size - 1 - i) and (t == T - big_blank_duration[i]) and (u == U - 1):
                        grad -= math.exp(alphas[col] + logpk - sigma - logll[mb])
            ...
            if (idx == blank_) and (t < T - 1):
                ...
                for i in range(num_big_blanks):
                    if (idx == alphabet_size - 1 - i) and (t < T - big_blank_duration[i]):
                        grad -= math.exp(
...
# # ## coding by sjguo6
# @cuda.jit()
# def compute_multiblank_fa_grad_kernel(
#     grads: torch.Tensor,
#     acts: torch.Tensor,
#     denom: torch.Tensor,
#     sigma: float,
...
#         #    such that all elements of the vocabulary size are covered in (V + 1 // thread_buffer) number of steps.
#         # As such, each thread will perform the while loop at least (V + 1 // thread_buffer) number of times
#         fastemit_lambda_new = emit_rate * (U-u) * fastemit_lambda
#         while idx < alphabet_size:
#             # remember, `col` represents the tri-index [b, t, u]
#             # therefore; logpk = denom[b, t, u] + acts[b, t, u, v]
#             logpk = denom[col] + acts[col * alphabet_size + idx]
...
#             # Once the gradient has been calculated, scale it by `fastemit_lambda`, as in Equation 10.
#             if fastemit_lambda > 0.0 and u < U - 1:
#                 fastemit_grad =  fastemit_lambda_new * math.exp(
#                     alphas[col]  # alphas(t, u)
#                     + (denom[col] + acts[col * alphabet_size + labels[u]])
#                     + betas[col + 1]  # betas(t, u+1)
#                     + logpk  # log Pr(k|t, u)
...
#                 # exp(log(1 + fastemit_lambda) + ...) is numerically more stable than
#                 # multiplying (1.0 + fastemit_lambda) with result.
#                 grad -= math.exp(math.log1p(fastemit_lambda_new)+
#                     alphas[col] + logpk - sigma - logll[mb] + betas[col + 1]
#                 )
...
# ## coding by sjguo6
@cuda.jit()
def compute_multiblank_fa_skip_grad_kernel(
    grads: torch.Tensor,
    acts: torch.Tensor,
    denom: torch.Tensor,
    sigma: float,
    ...
    Args:
        ...
        mlabels: Matrix of shape [B, U+1] (+1 here is due to <SOS> token - usually the RNNT blank).
            The matrix contains the padded target transcription that must be predicted.
        current_time: lower triangular matrix  [B, T, U-1]
        cumsum_time: upper triangular matrix for emit u  [B, T, U-1]
        minibatch: Int representing the batch size.
        maxT: The maximum possible acoustic sequence length. Represents T in the logprobs tensor.
        maxU: The maximum possible target sequence length. Represents U in the logprobs tensor.
        alphabet_size: The vocabulary dimension V+1 (inclusive of RNNT blank).
    ...
    if t < T and u < U:
        ...
        d_label = 0.0
        if fastemit_lambda > 0.0 and u < U - 1:
            for t_bar in range(0,T):
                for u_bar in range(0,U-1):
                    col_bar = offset + t_bar*maxU + u_bar
                    if t_bar + u_bar == t + u and current_time[mb,t_bar,u_bar] > 0: # t + u = n
                        # label [1,...,14839] ==> [1]
                        d_label += math.exp(
                            alphas[col_bar]  # alphas(t, u)
                            + (denom[col_bar] + acts[col_bar * alphabet_size + labels[u]]) #log Pr(k|t, u)
                            + betas[col_bar + 1]  # betas(t_bar, u_bar+1)
                            - sigma
                            - logll[mb]  # total log likelihood for normalization
                            )*current_time[mb,t_bar,u_bar]

        if u < U -1 and current_time[mb,t,u] > 0: 
             fastemit_lambda_new = 0.0
        else:
            fastemit_lambda_new = fastemit_lambda * emit_rate * (U - u + d_label) # 归一化
        # fastemit_lambda_new = fastemit_lambda
        while idx < alphabet_size:
...
#                 for u_bar in range(U-1):
#                     col_bar = offset + t_bar*maxU + u_bar
#                     if t_bar + u_bar == t + u and current_time[mb,t_bar,u_bar] > 0: # t + u = n
#                         # blank  [0] ==> [1] 
#                         if t_bar < T-1:
#                             d_blank += math.exp(
#                                 alphas[col_bar]  # alphas(t_bar, u_bar)
#                                 + (denom[col_bar] + acts[col_bar * alphabet_size]) #log Pr(k|t, u)
#                                 + betas[col_bar + maxU]  # betas(t_bar + 1, u_bar)
#                                 - sigma
#                                 - logll[mb]  # total log likelihood for normalization
#                                 )*current_time[mb,t_bar,u_bar]
#                         # big blank [14840,14841,14842] ==> [2,4,8]
#                         for i in range(num_big_blanks): 
#                             if t_bar < T - big_blank_duration[i]:
#                                 big_blank_idx = alphabet_size - 1 - i
#                                 ed_big_blank[i] += math.exp(
#                                     alphas[col_bar]  # alphas(t_bar, u_bar)
#                                     + (denom[col_bar] + acts[col_bar * alphabet_size + big_blank_idx]) #log Pr(k|t, u)
#                                     + betas[col_bar + big_blank_duration[i] * maxU]  # betas(t_bar+m, u_bar)
#                                     - sigma
#                                     - logll[mb]  # total log likelihood for normalization
#                                     )*current_time[mb,t_bar,u_bar]
...
#             if fastemit_lambda > 0.0 and u < U - 1:
#                 if current_time[mb,t,u] <= 0:
#                     fastemit_grad = - emit_rate * fastemit_lambda * (cumsum_time[mb,t,u]-d_label) * math.exp(
#                     alphas[col]  # alphas(t, u)
#                     + (denom[col] + acts[col * alphabet_size + labels[u]])
#                     + betas[col + 1]  # betas(t, u+1)
#                     + logpk  # log Pr(k|t, u)
...
#             if (idx == blank_) and (t < T - 1):
#                 grad -= math.exp( alphas[col] + logpk - sigma - logll[mb] + betas[col + maxU])
#                 # if current_time[mb,t,u] > 0 and u < U - 1:
#                 #     grad -= (1.0 - fastemit_lambda*(current_time[mb,t,u]-d_blank)) * math.exp( 
...
#                 for i in range(num_big_blanks):
#                     if (idx == alphabet_size - 1 - i) and (t < T - big_blank_duration[i]):
#                         grad -= math.exp(alphas[col] + logpk - sigma - logll[mb]  + betas[col + big_blank_duration[i] * maxU])
#                         # if current_time[mb,t,u] > 0 and u < U - 1:
#                         #     grad -= (1.0 - fastemit_lambda * (current_time[mb,t,u]-ed_big_blank[i])) * math.exp(
#                         #          alphas[col] + logpk - sigma - logll[mb]  + betas[col + big_blank_duration[i] * maxU])
#                         # else:
#                             # grad -= math.exp(alphas[col] + logpk - sigma - logll[mb]  + betas[col + big_blank_duration[i] * maxU])
...
#                 if  current_time[mb,t,u] <= 0:
#                     grad -= (1.0 - emit_rate * fastemit_lambda * (cumsum_time[mb,t,u]-d_label)) * math.exp( # 
#                     alphas[col] + logpk - sigma - logll[mb] + betas[col + 1]
#                     )
#                 else:
#                     grad -= math.exp( # 
