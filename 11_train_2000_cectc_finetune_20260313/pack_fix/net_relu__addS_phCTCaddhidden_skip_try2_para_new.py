...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.dnn_skip_out = ConvNoBN(256, 2, (1,1), (1,1), (0,0), 0, 0.1, "None")
    ...
    def forward(self, x: torch.Tensor, meta:Dict[str, torch.Tensor]):        
        ...
        if self.training:  
            ...
            def decode(enc_state, lengths):
                b = enc_state.shape[0]
                token = zero_token.repeat(b,1)
                token_list = -torch.ones((enc_state.shape[0], enc_state.shape[1]), device=enc_state.device, dtype = torch.int64)
                token_list_b = token_list.clone().to(dtype = torch.int64)
                logits_idx = torch.zeros((b,1),dtype=torch.int64).cuda()
                dec_state, hidden = self.decoder(token)
                dec_state = dec_state.to(torch.float16)
                for t in range(max(lengths)):
                    logits = self.joint(enc_state[:,t,:].detach().view(b,-1), dec_state.detach().view(b,-1))

                    logits_b = torch.cat((logits[:,:1], logits[:, -1:]),dim = -1)
            #         # print(logits_b)
                    out_b = F.softmax(logits_b, dim=-1).detach()
                    pred_b = torch.argmax(out_b, dim=-1)
            #         pred_b = int(pred_b.item())
                    token_list_b[:,t] = pred_b
            ...
            #         pred = int(pred.item())
                    ...
                    if torch.any(pred != 0):
                        token[pred != 0, :] = pred.view(b,-1)[pred != 0, :]

                        logits_idx[pred != 0] += 1
            #                 token = token.cuda()

                        new_dec_state, new_hidden = self.decoder(token, hidden=hidden)
                        dec_state[pred != 0,:,:] = new_dec_state[pred != 0,:,:].to(torch.float16)
                        hidden[0][0][0][pred != 0] = new_hidden[0][0][0][pred != 0]
                        hidden[0][1][0][pred != 0] = new_hidden[0][1][0][pred != 0]
                        hidden[1][0][0][pred != 0] = new_hidden[1][0][0][pred != 0]
                        hidden[1][1][0][pred != 0] = new_hidden[1][1][0][pred != 0]
            ...
            preds_rnnt, preds_rnnt_b = decode(enc_state.clone().detach(), inputs_length.int())
            ...
            if "label_ctc_ph" in meta:
                ...
                #print(phone_ctc_logit.shape, label_ctc[0]);exit()    # torch.Size([6, 319, 55]) torch.Size([6, 319])
                ...
                phone_ctc_loss = self.ctc_loss(phone_ctc_logit.permute(1,0,2).contiguous(),     #[T,B,C]
                                        label_ctc.contiguous(),    #[B,S]
                                        inputs_length.long().contiguous(), #[B]
                                        label_ctc_length.long().contiguous())  #[B]                
            ...
            cer_skip = self.total_dist_skip / self.total_word * 100
            ...
            skip_rate_enc = self.total_skip_count_enc/self.total_t_length
            skip_rate_dec = self.total_skip_count_dec/self.total_t_length
            ...
            if "label_ctc_ph" in meta:
                phctc_acc, _, _ = self.acc_ctc(phone_ctc_logit.squeeze(), label_ctc)
