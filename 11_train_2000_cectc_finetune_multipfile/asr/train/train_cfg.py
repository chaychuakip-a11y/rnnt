...
class Train():
    ...
    def __init_train(self):
        ...
        ###这里原先的层级不对
            ...
            for ind, data_add in enumerate(eval(self.__config["TrainDataSetting"]["List"])[1:]):
                if "pfile" == self.__config[data_add].get('DataType'):
                    if self.__config[data_add].get("ValidationDatadir"):
                        validation_datadir = self.__config[data_add]["ValidationDatadir"]
                        if not (self.__config[data_add].get("ValidationLabel")):
                            self.__raise_err("validation label isn't specified")
                        if not (self.__config[data_add].get("ValidationFeature")):
                            self.__raise_err("validation feature isn't specified")
                        validation_label_pfile_ = self.__config[data_add]["ValidationLabel"]
                        validation_feature_pfile_ = self.__config[data_add]["ValidationFeature"]
                        validation_label_pfile_ = os.path.join(validation_datadir, validation_label_pfile_)
                        validation_feature_pfile_ = os.path.join(validation_datadir, validation_feature_pfile_)
                        if not os.path.exists(validation_label_pfile_):
                            self.__raise_err("{0} doesn't exist".format(
                                validation_label_pfile_))
                        if not os.path.exists(validation_feature_pfile_):
                            self.__raise_err("{0} doesn't exist".format(
                                validation_feature_pfile_))
                        validation_start_sent_ = 0
                        validation_end_sent_ = self.data_infos[data_add]["CVSentNum"]
        ...
        if self.__config['TrainSetting'].get('TrainType') == "ASR":
            ...
            for ind, data_add in enumerate(eval(self.__config["TrainDataSetting"]["List"])[1:]):
                ...
                if self.__config[data_add].get('DataType') == "pfile":
                    ...
                    # print('DEBUG:!!!!!!!!! ', data_add, train_iternum_, len(PfileInfo(current_feature_pfile_).seq_info), start_sent_, end_sent_)
                ...
                if self.__is_inited:
                    train_iternum_ = train_iternum_ // self.__config["TrainSetting"].getint("NGpu")
        ...
        train_iternum = train_iternums[0]
        print("max(train_iternums): ", train_iternum)

        # print('self.__is_inited: ', self.__is_inited)
        # if self.__is_inited:
        #     train_iternum = train_iternum // self.__config["TrainSetting"].getint("NGpu") ### chu le liang bian
        ...
        if 0 == self.__rank:
            ...
            if self.__config['TrainSetting'].get('TrainType') == "ASR":
                ...
                for ind, data_add in enumerate(eval(self.__config["TrainDataSetting"]["List"])):
                    # print(data_add, self.data_infos[data_add]["val_start_sent"], self.data_infos[data_add]["val_end_sent"])
                    if self.__config[data_add].get('DataType') == "pfile":
            ...
            for ind, data_add in enumerate(eval(self.__config["TrainDataSetting"]["List"])):
                ...
                log += "{0}-{1}\t ".format(
                    data_add,self.data_infos[data_add]["train_iternum"])
            log += "\ntrain iternum.. {0}\n".format(
