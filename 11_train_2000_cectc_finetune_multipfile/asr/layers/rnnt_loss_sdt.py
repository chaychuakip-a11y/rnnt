...
def rnnt_loss_sdt(
    ...
    elif reduction == "mean_sdt":
        N_all = Nwers.shape[0]
        N = numsents.shape[0]
        #numsents = numsents.item()
        # print(N_all, N) #4*5, 4
        # print(negated_loss) #tensor([ 0.0717406869, -6.9220943451, -8.8132200241, -9.4324226379,
        s = 0
        # grad_scale = torch.zeros(N_all, dtype=torch.float32)
        costs_mwer = torch.zeros(N_all, dtype=negated_loss.dtype)
        # print('negated_loss: ', N, negated_loss.shape)
        for i in range(N):
            for j in range(numsents[i]):
                wer = Nwers[s+j]
                # grad_scale[s+j] = softmaxlogp[j]*(wer-wer_mean)
                costs_mwer[s+j] = negated_loss[s:s+numsents[i]]*wer
                # print(wer, softmaxlogp[j], nbest_lprobs[j], costs_mwer[s+j])#;exit() #tensor(-0.1250000000) tensor(4.6021661547e-05,
            s += numsents[i]
        negated_loss = costs_mwer
        for k in range(N_all):
            negated_loss[k] = negated_loss[k]/N_all
            # grads[k] = - grads[k]*grad_scale[k]/N_all

        # print("negated_loss: ", negated_loss)

        negated_loss = negated_loss.sum().unsqueeze_(-1)
        negated_loss = negated_loss.to(logits.device)
        return -negated_loss
    else:
        raise ValueError(
            f"reduction should be ('none' | 'mean' | 'sum' | 'mean_sdt'), given {reduction}"
        )
