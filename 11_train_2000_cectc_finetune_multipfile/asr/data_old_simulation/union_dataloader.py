...
def TestUnionDataLoader(data_infos, batch_num, bunchsize, maxsentframe, maxnumsent, ndivide=1, divide_index=0, nmod_pad=64, 
                    shuffle_batch=True, num_workers=1, cachesize=10000, random_seed=0, batch_ctrl=False, val=False):
    """
    data_infos = {
        "file_norm":"00",
    ...
    union_dataset = TestUnionDataset(split_data_infos, batch_num, bunchsize, maxsentframe, maxnumsent, nmod_pad, shuffle_batch, cachesize, random_seed, batch_ctrl, val)
...
class UnionDataset(Dataset):
    ...
    def __getitem__(self, index):
        # 读取原始数据
        read_samples = self.union_chunk_reader.gettrainbatch()
        while(read_samples is None):
            print("read_samples is None, circling")
            read_samples = self.union_chunk_reader.gettrainbatch()

        # 处理为特征数据
        datas, labels, label_ctcs = self.__get_samples(read_samples)

        # print(datas[0].shape, labels[0].shape, label_ctcs[0].shape);exit()
        
        # 将数据填充为self.nmod_pad倍数数据
        datas, data_mask = self.__pad_nmod(datas, self.nmod_pad, 0)
        labels, _ = self.__pad_nmod(labels, self.nmod_pad, -1)
        label_ctcs, _ = self.__pad_nmod(label_ctcs, self.nmod_pad, -1)
        ...
        meta["att_label"] = labels.long().contiguous()
        ...
        if label_ctcs != None:
            ...
            label_ctc = label_ctcs.squeeze()
            if label_ctc.dim() == 1:
                label_ctc = label_ctc.unsqueeze(0)

            label_ce = label_ctc.clone()
            try:
                label_ce = label_ce.permute(1,0)
            except Exception as e:
                print(e, label_ce.shape);exit()
            label_ce_mask = label_ce.clone()
            label_ce_mask[label_ce_mask >= 0] = 1
            label_ce_mask[label_ce_mask < 0] = 0

            label_ctc_ph = label_ctc.clone()
            ...
            if 1:
                ...
                for i in range(label_ctc.size(0)):
                    ...
                    label_ctc_ph[i, len(indices)+1] = 52
            ...
            try:
                ...
                print(e, label_ctc_ph.shape);exit()
            ...
            meta["label_ce"] = label_ce.long().contiguous()
            meta["label_ce_mask"] = label_ce_mask.long().contiguous()
            # print(label_ctc_ph[:,0], labels[:,0], label_ctc[0])
            # print(label_ctc_ph.shape,labels.shape,label_ctc.shape);exit()    #torch.Size([200, 40]) torch.Size([14, 40]) torch.Size([40, 200])
...
class TestUnionDataset(Dataset):
    def __init__(self, split_data_infos, batch_num, bunchsize, maxsentframe, maxnumsent, nmod_pad=64, shuffle_batch=True, cachesize=10000, random_seed=0, batch_ctrl=False,val=False):
        self.split_data_infos = split_data_infos
        ...
        if label_ctcs != None:
            ...
            if 1:
                ...
                for i in range(label_ctc.size(0)):
                    ...
                    # 遍历tensor的当前行
                    for j in range(label_ctc.size(1)):
                        if label_ctc[i, j] == 0 or label_ctc[i, j] == -1: ## or label_ctc[i, j] == 45 ## old version, sil=45 in english
                            continue
                        # 如果当前元素和上一个非重复元素不同
                        if label_ctc[i, j] != prev_value:
                            # 将当前元素的索引添加到列表中
