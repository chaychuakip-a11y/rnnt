...
import torch
from asr.layers.rnnt_loss.utils import global_constants, rnnt_helper
from asr.layers.rnnt_loss.utils.cuda_utils import gpu_rnnt_kernel, reduce
...
class MultiblankFAGPURNNT(GPURNNT):
    def __init__(
        self,
        sigma: float,
        num_big_blanks: int,
        ...
        self.emit_rate_ = emit_rate
        self.big_blank_workspace = cuda.as_cuda_array(
            big_blank_workspace
        )  # a flat vector of integer numbers that represents allocated memory slices
        self.ed_big_blank = cuda.as_cuda_array(
            ed_big_blank
        )
        self.num_big_blanks = num_big_blanks
        self.sigma = sigma
    ...
    def compute_cost_and_score(
        ...
        _, (denom, alphas, betas, llForward, llBackward, bigblank_durations, ed_big_blank) = self._prepare_fa_workspace()
        ...
        if training:
            ...
            grad_blocks_per_grid = self.minibatch_ * self.maxT_ * self.maxU_
            grad_threads_per_block = gpu_rnnt_kernel.GPU_RNNT_THREAD_SIZE
            # gpu_rnnt_kernel.compute_multiblank_fa_grad_kernel[
            #     grad_blocks_per_grid, grad_threads_per_block, self.stream_, 0
            # ](
            #     grads,
            #     acts,
            ...
            #     self.clamp_,
            # )
            gpu_rnnt_kernel.compute_multiblank_fa_skip_grad_kernel[
                grad_blocks_per_grid, grad_threads_per_block, self.stream_, 0
            ](
                grads,
                acts,
        ...
        # rnnt_helper.compute_costs_fa_data[blockspergrid, threadsperblock, self.stream_, 0](
        #     llForward, costs
        # )
        self.stream_.synchronize()
    ...
    def cost_and_grad(
        ...
        if (
            ...
            or cumsum_time is None
        ):
            return global_constants.RNNTStatus.RNNT_STATUS_INVALID_VALUE

        return self.compute_cost_and_score(acts, grads, costs, pad_labels, label_lengths, input_lengths,current_time,cumsum_time)
    ...
    def score_forward(
        ...
        if acts is None or costs is None or pad_labels is None or label_lengths is None or input_lengths is None or current_time is None or cumsum_time is not None :
            return global_constants.RNNTStatus.RNNT_STATUS_INVALID_VALUE

        return self.compute_cost_and_score(acts, None, costs, pad_labels, label_lengths, input_lengths,current_time,cumsum_time)

    def _prepare_fa_workspace(self) -> (int, Tuple[torch.Tensor]):
        """
        Helper method that uses the workspace and constructs slices of it that can be used.
        ...
        bigblank_durations = self.big_blank_workspace[: self.num_big_blanks]
        ed_big_blank = self.ed_big_blank[: self.num_big_blanks]

        return used_offset, (denom, alphas, betas, llForward, llBackward, bigblank_durations, ed_big_blank)
