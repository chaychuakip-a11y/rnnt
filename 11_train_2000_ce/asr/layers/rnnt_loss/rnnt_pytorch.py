...
from asr.layers.rnnt_loss import rnnt
from asr.layers.rnnt_loss.utils.cpu_utils import cpu_rnnt

__all__ = ['rnnt_loss', 'RNNTLossNumba', 'MultiblankRNNTLossNumba','MultiblankFARNNTLossNumba']
...
class _MultiblankFARNNTNumba(Function):
    """
    Numba class for multi-blank transducer loss (https://arxiv.org/pdf/2211.03541.pdf)
    """
    ...
    @staticmethod
    def forward(
        ctx, acts, labels, act_lens, label_lens, current_time, cumsum_time, blank, big_blank_durations, reduction, fastemit_lambda, emit_rate, clamp, sigma
    ):
        """
        big_blank_durations: list of durations for multi-blank transducer, e.g.
            [2, 4, 8].
        ...
        if is_cuda:
            loss_func = rnnt.multiblank_fa_rnnt_loss_gpu
        else:
            raise NotImplementedError()
    ...
    def backward(ctx, grad_output):
        if grad_output is not None and ctx.grads is not None:
            grad_output = grad_output.view(-1, 1, 1, 1).to(ctx.grads)
            return ctx.grads.mul_(grad_output), None, None, None, None, None, None, None, None, None, None, None, None, None
...
def multiblank_rnnt_loss(
    ...
    return _MultiblankRNNTNumba.apply(
        acts, labels, act_lens, label_lens, blank, big_blank_durations, reduction, fastemit_lambda, clamp, sigma
    )

def multiblank_fa_rnnt_loss(
    acts,
    labels,
    act_lens,
    label_lens,
    ...
    return _MultiblankFARNNTNumba.apply(
        acts, labels, act_lens, label_lens, current_time, cumsum_time, blank, big_blank_durations, reduction, fastemit_lambda, emit_rate, clamp, sigma
    )
...
class MultiblankFARNNTLossNumba(Module):
    """
    Parameters:
        blank (int): standard blank label.
        big_blank_durations: list of durations for multi-blank transducer, e.g.
    ...
    def __init__(
        ...
        super(MultiblankFARNNTLossNumba, self).__init__()
        self.blank = blank
        self.big_blank_durations = big_blank_durations
        self.fastemit_lambda = fastemit_lambda
        ...
        self.clamp = float(clamp) if clamp > 0 else 0.0
        self.reduction = reduction
        self.loss = _MultiblankFARNNTNumba.apply
        self.sigma = sigma

    def forward(self, acts, labels, act_lens, label_lens,current_time,cumsum_time):
        """
        log_probs: Tensor of (batch x seqLength x labelLength x outputDim) containing output from network
        labels: 2 dimensional Tensor containing all the targets of the batch with zero padded
        act_lens: Tensor of size (batch) containing size of each output sequence from the network
